...
import { ArrowRight, Building2, Check, Download, LoaderCircle, Maximize2, Minimize2, Pencil, Plus, RotateCcw, Trash2, Upload, X, ZoomIn, ZoomOut } from 'lucide-react'
...
export default function MatrixWorkspaceV8({ periodId, year, unitCode, unitName, canManage, onError, onNotice }: Props) {
  ...
  const selectedGuideline = useMemo(() => {
    if (!groupedLayout || !selectedMatrix || !selectedArea) return null
    ...
    return null
  }, [groupedLayout, selectedMatrix, selectedArea, guidelines, managementById])
  ...
  async function loadWorkspace() {
    ...
    try {
      ...
      ;(catalogResult.data || []).forEach(item => {
        ...
        if (area && !uniqueAreas.has(normalizeText(area.name))) uniqueAreas.set(normalizeText(area.name), area)
      })
  ...
  async function exportExcel() {
    ...
    try {
      ...
      const exportRows = rows.map((row, index) => ({ ...(!groupedLayout ? { 'N°': index + 1 } : {}), ...(groupedLayout ? { Objetivo: row.objective_group || '' } : {}), Acción: row.objective || '', Responsable: row.responsible_manager_id ? managerById.get(row.responsible_manager_id)?.name || row.responsible_text || '' : row.responsible_text || '', Prioridad: row.priority || '', 'Hitos / Fechas': row.milestones || '', 'KPI (Cuantitativo)': row.kpi || '', Inicio: row.start_date || '', Fin: row.end_date || '', 'Riesgos de no ejecutar': row.risks || '', Restricciones: row.restrictions || '', Soporte: row.support || '', Entregable: row.deliverables || '', Comité: row.committee || '' }))
  ...
  const tableColSpan = (groupedLayout ? 12 : 13) + (canManage ? 1 : 0)
  ...
  return <div className={`matrix-v5 matrix-v5--${unitAccent[unitCode]} ${page === 'sheet' ? 'matrix-v5--sheet' : ''} ${expanded ? 'matrix-v5--expanded' : ''}`}>
    ...
    {page === 'sheet' && selectedMatrix && <section className="matrix-v5-plan-shell">
      ...
      {groupedLayout && <section className="matrix-v5-guidelines-card"><div className="matrix-v5-guidelines-head"><div><span>LINEAMIENTOS</span><h3>Lineamiento de la matriz</h3></div></div><div className="matrix-v5-guidelines-scroll"><table><thead><tr><th>Lineamiento estratégico</th></tr></thead><tbody><tr><td><strong>{lineamientoText}</strong></td></tr></tbody></table></div></section>}
