...
type TesseractApi = {
  recognize: (image: File | HTMLCanvasElement, lang: string, options?: { logger?: (message: { status?: string; progress?: number }) => void }) => Promise<{ data: { text: string } }>
}
...
function loadScript(key: string, src: string, ready: () => boolean) {
  ...
  }).catch(error => { scriptPromises.delete(key); throw error })
...
async function loadXlsx(): Promise<XlsxApi> {
  ...
  if (!xlsxPromise) xlsxPromise = loadScript('Excel', 'https://cdn.jsdelivr.net/npm/xlsx@0.18.5/dist/xlsx.full.min.js', () => Boolean(get())).then(() => get() as XlsxApi).catch(error => { xlsxPromise = null; throw error })
  return xlsxPromise
}
...
async function loadPdf(): Promise<PdfApi> {
  ...
  if (!pdfPromise) pdfPromise = loadScript('lector PDF', 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js', () => Boolean(get())).then(() => {
...
async function loadZip(): Promise<ZipApi> {
  ...
  if (!zipPromise) zipPromise = loadScript('lector PowerPoint', 'https://cdn.jsdelivr.net/npm/jszip@3.10.1/dist/jszip.min.js', () => Boolean(get())).then(() => get() as ZipApi).catch(error => { zipPromise = null; throw error })
  return zipPromise
}
...
async function loadTesseract(): Promise<TesseractApi> {
  ...
  if (!tesseractPromise) tesseractPromise = loadScript('lector de imágenes', 'https://cdn.jsdelivr.net/npm/tesseract.js@5/dist/tesseract.min.js', () => Boolean(get())).then(() => get() as TesseractApi).catch(error => { tesseractPromise = null; throw error })
...
function findHeaderIndex(row: unknown[], patterns: RegExp[]) { return row.findIndex(cell => patterns.some(pattern => pattern.test(normalize(asText(cell))))) }
...
function parseWorkbookRows(matrix: unknown[][], managements: Management[], managers: Manager[], unitCode: string, defaultManagementId?: string | null) {
  ...
  if (guidelineIndex < 0) guidelineIndex = header.length >= 3 ? 2 : 0
  ...
  return matrix.slice(headerRow + 1).flatMap((row, index) => {
    ...
    if (![category, rawNumber, text, managementText, managerText].some(Boolean)) return []
    if (/^lineamientos? estrategicos?$/i.test(text)) return []
    const numeric = rawNumber.match(/\d{1,3}/)?.[0]
    const code = unitCode === 'CENTRAL' ? rawNumber : /^L\d+/i.test(rawNumber) ? rawNumber.toUpperCase().replace(/\s+/g, '') : numeric ? `L${numeric}` : ''
    return [{ id: `${Date.now()}-${index}`, enabled: true, category, code, text: unitCode === 'CENTRAL' ? text : text.replace(/^\s*L\d+\s*:\s*/i, '').trim(), managementId: matchCatalog(managementText, areaCandidates) || defaultManagementId || '', responsibleId: matchCatalog(managerText, managerCandidates) }]
  })
}

function cleanExtractedLines(text: string) {
  return text.split(/\r?\n/).map(line => line.replace(/[\t\u00a0]+/g, ' ').replace(/\s{2,}/g, ' ').trim()).filter(Boolean)
}
...
function parseExtractedText(text: string, unitCode: string, defaultManagementId?: string | null) {
  ...
  let pendingCode = ''
  let category = ''
  const ignore = /^(?:n[°ºo]?|categoria|categoría|lineamientos?(?: estrategicos?)?|gerencia responsable|gerente responsable|estado|acciones)$/i
  const categoryPrefix = /^(?:categoria|categoría)\s*[:.-]\s*(.+)$/i
  const numbered = /^(?:L\s*)?(\d{1,3})\s*(?:[:.)-]\s*)?(.{3,})$/i
  const onlyNumber = /^(?:L\s*)?(\d{1,3})\s*[:.)-]?$/i
  const push = () => { if (current && (current.code.trim() || current.text.trim() || current.category.trim())) rows.push(current); current = null }
  lines.forEach((line, index) => {
    if (ignore.test(line)) return
    const categoryMatch = line.match(categoryPrefix)
    if (categoryMatch) { category = categoryMatch[1].trim(); return }
    const single = line.match(onlyNumber)
    if (single) { push(); pendingCode = single[1]; return }
    const match = line.match(numbered)
    if (match) {
      push()
      const numeric = match[1]
      current = { id: `${Date.now()}-${index}`, enabled: true, category, code: unitCode === 'CENTRAL' ? numeric : `L${numeric}`, text: match[2].trim(), managementId: defaultManagementId || '', responsibleId: '' }
      pendingCode = ''
      return
    }
    if (pendingCode) {
      current = { id: `${Date.now()}-${index}`, enabled: true, category, code: unitCode === 'CENTRAL' ? pendingCode : `L${pendingCode}`, text: line, managementId: defaultManagementId || '', responsibleId: '' }
      pendingCode = ''
      return
    }
    if (current) {
      if (!/^(?:aprobado|pendiente|en revision|en revisión)$/i.test(line)) current.text = `${current.text} ${line}`.replace(/\s+/g, ' ').trim()
      return
    }
    if (line.length >= 18 && !/^(?:periodo|unidad|area|área|responsable)/i.test(line)) {
      current = { id: `${Date.now()}-${index}`, enabled: true, category, code: '', text: line, managementId: defaultManagementId || '', responsibleId: '' }
    }
  })
  push()
  return rows
...
async function extractPdfText(file: File, setProgress: (message: string) => void) {
  ...
  for (let pageNo = 1; pageNo <= doc.numPages; pageNo += 1) {
    ...
    const lines = [...grouped.entries()].sort((a, b) => b[0] - a[0]).map(([, values]) => values.sort((a, b) => a.x - b.x).map(item => item.text).join(' '))
  ...
  for (let pageNo = 1; pageNo <= pages; pageNo += 1) {
    ...
    const result = await ocr.recognize(canvas, 'spa', { logger: message => { if (message.status === 'recognizing text' && typeof message.progress === 'number') setProgress(`PDF escaneado · página ${pageNo}/${pages} · ${Math.round(message.progress * 100)}%`) } })
...
async function extractPptxText(file: File, setProgress: (message: string) => void) {
  ...
  for (let index = 0; index < slideNames.length; index += 1) {
    ...
    const lines = paragraphs.map(paragraph => Array.from(paragraph.getElementsByTagName('a:t')).map(node => node.textContent || '').join('').trim()).filter(Boolean)
...
async function extractImageText(file: File, setProgress: (message: string) => void) {
  ...
  const result = await api.recognize(file, 'spa', { logger: message => {
...
export default function GuidelineMultiImport({ unit, periodId, open, onClose, onImported, defaultManagementId }: Props) {
  ...
  useEffect(() => { if (!open) { setRows([]); setFileName(''); setError(''); setProgress(''); setLoading(false); setSaving(false) } }, [open])
  ...
  async function processFile(file: File) {
    ...
    try {
      ...
      if (/\.(xlsx|xls)$/.test(lower)) {
        ...
        parsed = parseExtractedText(text, unit.code, defaultManagementId)
      } else if (/\.pptx$/.test(lower)) {
        const text = await extractPptxText(file, setProgress)
        parsed = parseExtractedText(text, unit.code, defaultManagementId)
  ...
  function patchRow(id: string, patch: Partial<DraftRow>) { setRows(current => current.map(row => row.id === id ? { ...row, ...patch } : row)) }

  async function importRows() {
    if (!supabase || !periodId || !activeRows.length) return
    const missingArea = activeRows.find(row => !row.managementId)
    if (missingArea) { setError('Asigna un área a todas las filas que vas a importar.'); return }
    setSaving(true); setError('')
    try {
      const { data: existing, error: existingError } = await supabase.from('planning_guidelines').select('guideline_text,code,category').eq('period_id', periodId).eq('unit_code', unit.code)
      ...
      const payload = activeRows.map((row, index) => {
        ...
        if (unit.code === 'CENTRAL') {
          const code = row.code.trim() || null
          const guidelineText = row.text.trim()
          return { period_id: periodId, unit_code: unit.code, management_id: row.managementId, category, code, guideline_text: guidelineText, responsible_manager_id: row.responsibleId || null, active: true, sort_order: baseOrder + index, _key: normalize(`${category || ''}|${code || ''}|${guidelineText}`) }
        }
        ...
        return { period_id: periodId, unit_code: unit.code, management_id: row.managementId, category, code, guideline_text: fullText, responsible_manager_id: row.responsibleId || null, active: true, sort_order: baseOrder + index, _key: normalize(`${category || ''}|${code}|${fullText}`) }
      ...
      onImported(insertPayload.length); onClose()
    } catch (cause) {
  ...
  return <div className="guideline-import-backdrop" role="presentation" onMouseDown={event => { if (event.currentTarget === event.target && !loading && !saving) onClose() }}>
    <section className="guideline-import-dialog" role="dialog" aria-modal="true">
      ...
      <header className="guideline-import-header"><div className="guideline-import-icon"><FileText size={22}/></div><div><span>Importación inteligente</span><h3>Importar lineamientos</h3><p>Extrae lineamientos desde Excel, PDF, PowerPoint o imágenes. Siempre podrás revisar y corregir lo detectado antes de guardar.</p></div></header>
      <div className="guideline-import-context"><strong>{unit.code} · {unit.name}</strong><span>Se guardarán únicamente en el periodo actual.</span></div>
      ...
      <button className="guideline-import-picker" type="button" onClick={() => inputRef.current?.click()} disabled={loading || saving}><span><FileSpreadsheet size={26}/></span><div><strong>{fileName || 'Seleccionar archivo'}</strong><small>Excel · PDF · PowerPoint · PNG/JPG/WEBP · máximo 50 MB</small></div><b>{loading ? <LoaderCircle className="spin" size={18}/> : 'Elegir archivo'}</b></button>
      ...
      {rows.length > 0 && <div className="guideline-import-preview">
        <div className="guideline-import-preview-head"><div><strong>Vista previa antes de guardar</strong><small>{activeRows.length} seleccionados</small></div><span>Corrige cualquier lectura de PDF, PPT o imagen antes de confirmar.</span></div>
        <div className="guideline-import-table-wrap"><table><thead><tr><th>Usar</th>{unit.code === 'CENTRAL' && <th>Categoría</th>}<th>N°</th><th>Lineamiento</th><th>Área</th><th>Responsable</th><th></th></tr></thead>
          <tbody>{rows.map(row => <tr key={row.id} className={!row.enabled ? 'disabled' : ''}><td><input type="checkbox" checked={row.enabled} onChange={event => patchRow(row.id, { enabled: event.target.checked })}/></td>{unit.code === 'CENTRAL' && <td><input value={row.category} onChange={event => patchRow(row.id, { category: event.target.value })}/></td>}<td><input className="code" value={row.code} onChange={event => patchRow(row.id, { code: event.target.value })}/></td><td><textarea value={row.text} onChange={event => patchRow(row.id, { text: event.target.value })}/></td><td><select value={row.managementId} onChange={event => patchRow(row.id, { managementId: event.target.value })}><option value="">Seleccionar...</option>{managements.filter(item => item.active).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></td><td><select value={row.responsibleId} onChange={event => patchRow(row.id, { responsibleId: event.target.value })}><option value="">Sin asignar</option>{managers.filter(item => item.active).map(item => <option key={item.id} value={item.id}>{item.name}{item.cargo ? ` · ${item.cargo}` : ''}</option>)}</select></td><td><button type="button" className="remove" onClick={() => setRows(current => current.filter(item => item.id !== row.id))}><Trash2 size={15}/></button></td></tr>)}</tbody></table></div>
      </div>}
