...
const roleValue = (role: string | null) => role || 'AREA'
...
export default function PermissionCatalogV4({ units, canManage }: Props) {
  ...
  async function updateRole(user: PermissionUser, nextRole: string) {
    if (!supabase || !canManage) return
    ...
    setNotice(nextRole === 'AREA' ? `${user.full_name || user.email} ahora usa permisos por área.` : `Rol actualizado correctamente.`)
    await loadData()
  }

  async function updatePermission(user: PermissionUser, mode: 'view' | 'edit', enabled: boolean) {
    if (!supabase || !canManage || !targetArea || user.global_role) return
  ...
  async function applyBulk(action: BulkAction) {
    ...
    if (!areaUsers.length) {
      setError('Los usuarios seleccionados tienen acceso global. Cambia su Rol principal a “Acceso por área” para administrar áreas específicas.')
      return
    }
    ...
    setNotice(`${areaUsers.length} usuario${areaUsers.length === 1 ? '' : 's'} actualizado${areaUsers.length === 1 ? '' : 's'}${skipped ? `. ${skipped} con acceso global no fue modificado.` : '.'}`)
  ...
  return <section className={`permission-v2 permission-v4 config-accordion ${open ? 'open' : ''}`}>
    <button className="config-accordion-head" type="button" onClick={() => setOpen(value => !value)}>
      ...
      <div><small>SEGURIDAD Y ACCESO</small><h2>Panel de Control de Permisos por Rol y Área</h2><p>Asigna roles y permisos por usuario, unidad y área desde un solo panel.</p></div>
    ...
    {open && <div className="config-accordion-body permission-v2-body">
      {!canManage ? <div className="permission-v2-empty">Solo Gestión Estratégica puede administrar estos permisos.</div> : loading ? <div className="permission-v2-loading"><LoaderCircle className="spin" size={20}/> Cargando usuarios...</div> : <>
        ...
        <div className="permission-v4-help"><strong>Cómo editar permisos:</strong><span>1) Cambia “Rol principal” a <b>Acceso por área</b>. 2) Elige Unidad y Área arriba. 3) Activa Acceso y Edición. Los roles globales tienen acceso total y por eso no usan permisos individuales.</span></div>
        ...
        <div className="permission-v2-toolbar">
          ...
          <div className="permission-v2-bulk-wrap"><button type="button" className="permission-v2-bulk" disabled={!selectedIds.size || !targetArea || bulkSaving} onClick={() => setBulkOpen(value => !value)}>{bulkSaving ? <LoaderCircle className="spin" size={14}/> : <UsersRound size={15}/>} Acciones en masa</button>{bulkOpen && <div className="permission-v2-bulk-menu"><button onClick={() => void applyBulk('VIEW')}>Dar acceso</button><button onClick={() => void applyBulk('EDIT')}>Dar acceso + edición</button><button className="danger" onClick={() => void applyBulk('REMOVE')}>Quitar acceso</button></div>}</div>
        </div>

        <div className="permission-v2-context"><div><strong>{targetArea ? `${unitCode} · ${targetArea.name}` : `${unitCode} · Sin área activa`}</strong><span>Los interruptores modifican el área seleccionada.</span></div><div><strong>{users.length}</strong><span>Usuarios totales</span></div><div><strong>Página {page} de {totalPages}</strong><span>{filteredUsers.length} resultados</span></div></div>
        ...
        <div className="permission-v2-table-wrap"><table className="permission-v2-table"><thead><tr><th className="permission-v2-check"><input type="checkbox" checked={allPageSelected} onChange={toggleSelectAll}/></th><th>Usuario</th><th>Correo electrónico</th><th>Rol principal</th><th>Áreas asignadas</th><th>Acceso</th><th>Edición</th></tr></thead><tbody>
          {pageUsers.length === 0 ? <tr><td colSpan={7} className="permission-v2-empty">No encontramos usuarios con esos filtros.</td></tr> : pageUsers.map(user => {
            ...
            return <tr key={user.id} className={selectedIds.has(user.id) ? 'selected' : ''}>
              ...
              <td><select className="permission-v4-role-select" value={roleValue(user.global_role)} disabled={savingKey.startsWith(user.id)} onChange={event => void updateRole(user, event.target.value)}><option value="GESTION_ESTRATEGICA">Gestión Estratégica</option><option value="GERENTE_GENERAL">Gerente General</option><option value="AREA">Acceso por área</option></select></td>
        ...
        <div className="permission-v2-footer"><div className="permission-v2-global-units"><strong>Acceso global</strong>{unitOptions.map(unit => <span key={unit.code} className={`unit-${unit.code.toLowerCase()}`}>{unit.code}</span>)}</div><div className="permission-v2-pagination"><button disabled={page <= 1} onClick={() => setPage(value => Math.max(1, value - 1))}><ChevronLeft size={16}/></button><span>{page}</span><button disabled={page >= totalPages} onClick={() => setPage(value => Math.min(totalPages, value + 1))}><ChevronRight size={16}/></button></div></div>
      </>}
    </div>}
  </section>
}
