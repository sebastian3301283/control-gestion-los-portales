...
export default function MatrixWorkspace({ periodId, year, unitCode, unitName, canManage, onError, onNotice }: Props) {
  ...
  async function addArea(managementId: string) {
    ...
    const { error } = await supabase.from('matrix_area_selections').insert({
      period_id: periodId,
      unit_code: unitCode,
      directory_group: directoryGroup,
      management_id: managementId,
      sort_order: areaSelections.length,
    })
    setSaving(false)
    if (error) { onError(error.code === '23505' ? 'Esa área ya fue añadida.' : 'No pudimos añadir el área.'); return }
    setAreaPickerOpen(false)
    onNotice('Área añadida a las matrices.')
    await loadWorkspace()
  }
  ...
  function chooseProcess(id: string) {
    setSelectedProcessId(id); setSelectedMatrixId(''); setRows([]); cancelRowEdit(); setPage('matrices')
  }

  function chooseMatrix(id: string) {
    setSelectedMatrixId(id); cancelRowEdit(); setPage('sheet')
  }
  ...
  async function saveRow() {
    ...
    cancelRowEdit(); onNotice(wasEditing ? 'Fila actualizada.' : 'Fila agregada a la matriz.'); await loadRows(selectedMatrix.id)
  }
  ...
  if (unitCode === 'CENTRAL') {
    return (
      <section className="matrix-placeholder matrix-placeholder--central">
        ...
        <div><small>Etapa de matrices</small><h3>Central se configurará después</h3><p>Primero construiremos las matrices de VS, HU, Departamentos y Hoteles.</p></div>
      </section>
    )
  }
    ...
    <div className={`matrix-workspace matrix-workspace--${unitAccent[unitCode]}`}>
      <section className="matrix-intro">
        ...
        <div className="matrix-route">
          ...
          <span className={page === 'matrices' || page === 'sheet' ? 'done' : ''}>2. Matrices</span>
          <ChevronRight size={15}/>
          <span className={page === 'sheet' ? 'active' : ''}>3. Edición</span>
        </div>
      </section>
      ...
      {loading ? <div className="matrix-loading"><LoaderCircle className="spin" size={22}/> Cargando estructura...</div> : <>
        ...
        {page === 'processes' && selectedArea && <section className="matrix-stage">
          <div className="matrix-stage-head"><div><small>{selectedArea.name}</small><h4>Procesos</h4><p>Selecciona un proceso para continuar a sus matrices.</p></div>{canManage && <button className="matrix-primary" onClick={() => setProcessFormOpen(true)}><Plus size={15}/> Nuevo proceso</button>}</div>
        ...
        {page === 'matrices' && selectedProcess && <section className="matrix-stage">
          <div className="matrix-stage-head"><div><small>{selectedArea?.name} · {selectedProcess.name}</small><h4>Matrices del proceso</h4><p>Abre una matriz para trabajarla en una pantalla independiente.</p></div>{canManage && <button className="matrix-primary" onClick={() => { setMatrixName(`Matriz de ${selectedProcess.name}`); setMatrixFormOpen(true) }}><Plus size={15}/> Nueva matriz</button>}</div>
