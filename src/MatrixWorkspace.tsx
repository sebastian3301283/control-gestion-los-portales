...
export default function MatrixWorkspace({ periodId, year, unitCode, unitName, canManage, onError, onNotice }: Props) {
  ...
  async function loadManagers(){
    ...
    const results=await Promise.all(queries); if(results.some(result=>result.error)) throw new Error('MANAGER_LOAD')
    const unique=new Map<string,Manager>(); results.forEach(result=>(result.data||[]).forEach(item=>unique.set(String(item.id),item as Manager)))
  ...
  async function loadWorkspace(){
    ...
    try{
      const areaQuery = unitCode==='HU'
        ? supabase.from('managements_global').select('id,name,unit_code,directory_group').eq('unit_code','HU').eq('active',true).order('name')
        : supabase.from('managements_global').select('id,name,unit_code,directory_group').eq('unit_code',unitCode).eq('active',true).order('name')
      const [areaResult,processResult,matrixResult,managerList]=await Promise.all([
        areaQuery,
      ...
      if(areaResult.error||processResult.error||matrixResult.error) throw new Error('LOAD')
      const uniqueAreas=new Map<string,Area>()
      ;((areaResult.data||[]) as Area[]).forEach(area=>{ const key=normalize(area.name); const current=uniqueAreas.get(key); if(!current||area.directory_group==='HU') uniqueAreas.set(key,area) })
    ...
    }catch{ onError('No pudimos cargar las áreas y matrices de esta unidad.') } finally { setLoading(false) }
  }

  async function loadRows(matrixId:string){
    if(!supabase) return
    setRowsLoading(true); const {data,error}=await supabase.from('matrix_rows').select('*').eq('matrix_id',matrixId).order('sort_order').order('created_at'); setRowsLoading(false)
    if(error){onError('No pudimos cargar la matriz.');return} setRows((data||[]) as MatrixRow[])
  }

  function matricesForArea(areaId:string){ const ids=new Set(processes.filter(item=>item.management_id===areaId).map(item=>item.id)); return matrices.filter(item=>ids.has(item.process_id)) }
  ...
  async function ensureProcess(area:Area){
    ...
    const {data,error}=await supabase.from('processes').insert({ unit_code:unitCode, directory_group:area.directory_group, management_id:area.id, name:'Matriz general', description:'Proceso automático para matrices', sort_order:0 }).select('id,name,description,management_id,unit_code,directory_group,sort_order').single()
    if(error||!data) return null
    const created=data as Process; setProcesses(current=>[...current,created]); return created
  }

  async function createMatrix(event:FormEvent){
    event.preventDefault(); if(!supabase||!canManage||!selectedArea) return
    const name=matrixName.trim()||`Matriz de ${selectedArea.name}`; setSaving(true); onError(''); onNotice('')
    ...
    const {data,error}=await supabase.from('matrices').insert({ period_id:periodId, unit_code:unitCode, directory_group:selectedArea.directory_group, process_id:process.id, name, description:matrixDescription.trim()||null, status:'DRAFT', sort_order:currentCount }).select('id').single()
    setSaving(false)
    if(error||!data){onError(error?.code==='23505'?'Ya existe una matriz con ese nombre en esta área.':'No pudimos generar la matriz.');return}
    setMatrixFormOpen(false); setMatrixName(''); setMatrixDescription(''); await loadWorkspace(); setSelectedMatrixId(String(data.id)); setPage('sheet'); onNotice('Matriz generada correctamente.')
  }

  function startNewRow(){setEditingRowId(null);setRowDraft(emptyRow);setRowFormOpen(true);onError('');onNotice('')}
  function startEditRow(row:MatrixRow){setEditingRowId(row.id);setRowDraft({objective:row.objective||'',action_plan:row.action_plan||'',responsible_manager_id:row.responsible_manager_id,responsible_text:row.responsible_text||'',priority:row.priority||'',milestones:row.milestones||'',kpi:row.kpi||'',target:row.target||'',start_date:row.start_date||'',end_date:row.end_date||'',risks:row.risks||'',restrictions:row.restrictions||'',support:row.support||'',deliverables:row.deliverables||'',committee:row.committee||'',status:row.status||'DRAFT'});setRowFormOpen(true)}
  ...
  async function saveRow(){
    ...
    setSaving(true); onError(''); onNotice(''); const manager=rowDraft.responsible_manager_id?managerById.get(rowDraft.responsible_manager_id):null
    ...
    const result=editingRowId?await supabase.from('matrix_rows').update(payload).eq('id',editingRowId):await supabase.from('matrix_rows').insert(payload); setSaving(false)
    if(result.error){onError('No pudimos guardar la fila.');return} const wasEditing=Boolean(editingRowId); cancelRowEdit(); onNotice(wasEditing?'Fila actualizada.':'Fila agregada.'); await loadRows(selectedMatrix.id)
  }
  async function deleteRow(id:string){if(!supabase||!canManage||!selectedMatrix)return;const{error}=await supabase.from('matrix_rows').delete().eq('id',id);if(error){onError('No pudimos eliminar la fila.');return}onNotice('Fila eliminada.');await loadRows(selectedMatrix.id)}
  ...
  return <div className={`matrix-workspace matrix-workspace--${unitAccent[unitCode]}`}>
    <section className="matrix-intro"><div><span className="matrix-kicker">Periodo {year} · {unitCode}</span><h3>Matrices de {unitName}</h3><p>Las áreas se administran en <strong>Configuración → Edición de áreas</strong>. Aquí solo eliges el área y generas su matriz.</p></div><div className="matrix-route"><button className={page==='areas'?'active':'done'} onClick={()=>{setPage('areas');setSelectedAreaId('');setSelectedMatrixId('')}}>1. Área</button><ChevronRight size={15}/><span className={page==='matrices'?'active':page==='sheet'?'done':''}>2. Matrices</span><ChevronRight size={15}/><span className={page==='sheet'?'active':''}>3. Edición</span></div></section>
    ...
    {loading?<div className="matrix-loading"><LoaderCircle className="spin" size={22}/> Cargando áreas y matrices...</div>:<>
      {page==='areas'&&<section className="matrix-stage"><div className="matrix-stage-head"><div><small>Áreas configuradas</small><h4>Genera una matriz directamente</h4><p>Ya no necesitas crear procesos. Las áreas de esta unidad se gestionan desde Configuración.</p></div></div>{areas.length===0?<div className="matrix-empty"><Building2 size={24}/><strong>No hay áreas configuradas</strong><span>Ve a Configuración → Edición de áreas y crea las áreas de {unitName}.</span></div>:<div className="matrix-area-grid">{areas.map(area=>{const count=matricesForArea(area.id).length;return <div className="matrix-area-card matrix-area-card--direct" key={area.id}><button className="matrix-area-open" onClick={()=>openArea(area)}><span><Building2 size={20}/></span><div><strong>{area.name}</strong><small>{count} matriz{count===1?'':'ces'} generada{count===1?'':'s'}</small></div><ArrowRight size={17}/></button>{canManage&&<button className="matrix-generate" onClick={()=>startMatrix(area)}><Plus size={14}/> Generar matriz</button>}</div>})}</div>}</section>}
      {page==='matrices'&&selectedArea&&<section className="matrix-stage"><div className="matrix-stage-head"><div><small>{selectedArea.name}</small><h4>Matrices del área</h4><p>Abre una matriz existente o genera una nueva directamente.</p></div>{canManage&&<button className="matrix-primary" onClick={()=>startMatrix(selectedArea)}><Plus size={15}/> Generar matriz</button>}</div>{areaMatrices.length===0?<div className="matrix-empty compact"><ClipboardList size={22}/><strong>Aún no hay matrices</strong><span>Presiona “Generar matriz” para crear la primera.</span></div>:<div className="matrix-simple-table-wrap"><table className="matrix-simple-table"><thead><tr><th>N°</th><th>Matriz</th><th>Descripción</th><th>Estado</th><th></th></tr></thead><tbody>{areaMatrices.map((matrix,index)=><tr key={matrix.id}><td>{index+1}</td><td><strong>{matrix.name}</strong></td><td>{matrix.description||'Matriz editable'}</td><td><span className={`matrix-status matrix-status--${matrix.status.toLowerCase()}`}>{statusLabel(matrix.status)}</span></td><td><button onClick={()=>chooseMatrix(matrix.id)}>Abrir <ArrowRight size={14}/></button></td></tr>)}</tbody></table></div>}</section>}
      {page==='sheet'&&selectedMatrix&&<section className="matrix-sheet-card"><div className="matrix-sheet-head"><div><small>{selectedArea?.name}</small><h4>{selectedMatrix.name}</h4><p>Edita la matriz directamente como una hoja de trabajo.</p></div>{canManage&&<button className="matrix-primary" onClick={startNewRow}><Plus size={15}/> Nueva fila</button>}</div><div className="matrix-sheet-scroll"><table className="matrix-sheet"><thead><tr><th>N°</th><th>Objetivo</th><th>Plan de acción</th><th>Responsable</th><th>Prioridad</th><th>Hitos</th><th>KPI</th><th>Objetivo KPI</th><th>Fecha inicio</th><th>Fecha fin</th><th>Riesgos</th><th>Restricciones</th><th>Soporte</th><th>Entregables</th><th>Comité</th>{canManage&&<th>Acciones</th>}</tr></thead><tbody>
