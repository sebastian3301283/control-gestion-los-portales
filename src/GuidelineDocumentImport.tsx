...
function parseWorkbookRows(matrix: unknown[][], managements: Management[], managers: Manager[], unitCode: string, defaultManagementId?: string | null) {
  ...
  return matrix.slice(headerRow + 1).flatMap((row, index) => {
    ...
    const code = unitCode === 'CENTRAL'
      ? rawNumber
      : /^L\d+/i.test(rawNumber) ? rawNumber.toUpperCase().replace(/\s+/g, '') : numeric ? `L${numeric}` : ''
    ...
    return [{
      ...
      category,
      code,
      text: unitCode === 'CENTRAL' ? text : text.replace(/^\s*L\d+\s*:\s*/i, '').trim(),
...
export default function GuidelineDocumentImport({ unit, periodId, open, onClose, onImported, defaultManagementId }: Props) {
  ...
  async function processFile(file: File) {
    setError(''); setRows([]); setFileName(file.name); setLoading(true); setProgress('Leyendo Excel...')
    try {
      ...
      const parsed = parseWorkbookRows(matrix, managements, managers, unit.code, defaultManagementId)
      if (!parsed.length) throw new Error('No pudimos identificar filas de lineamientos en el Excel.')
      setRows(parsed)
      setProgress(`${parsed.length} lineamiento${parsed.length === 1 ? '' : 's'} detectado${parsed.length === 1 ? '' : 's'}. Revisa la tabla antes de guardar.`)
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'No pudimos leer el Excel.'); setProgress('')
  ...
  return <div className="guideline-import-backdrop" role="presentation" onMouseDown={event => { if (event.currentTarget === event.target && !loading && !saving) onClose() }}>
    <section className="guideline-import-dialog" role="dialog" aria-modal="true">
      ...
      <header className="guideline-import-header"><div className="guideline-import-icon"><FileSpreadsheet size={22}/></div><div><span>Carga desde Excel</span><h3>Importar lineamientos</h3><p>Selecciona un Excel, revisa las filas detectadas y confirma antes de guardarlas.</p></div></header>
      ...
      <button className="guideline-import-picker" type="button" onClick={() => inputRef.current?.click()} disabled={loading || saving}><span><FileSpreadsheet size={26}/></span><div><strong>{fileName || 'Seleccionar archivo Excel'}</strong><small>XLSX o XLS · máximo 18 MB</small></div><b>{loading ? <LoaderCircle className="spin" size={18}/> : 'Elegir Excel'}</b></button>
      ...
      {rows.length > 0 && <div className="guideline-import-preview">
        ...
        <div className="guideline-import-table-wrap"><table>
          <thead><tr><th>Usar</th>{unit.code === 'CENTRAL' && <th>Categoría</th>}<th>N°</th><th>Lineamiento</th><th>Área</th><th>Responsable</th><th></th></tr></thead>
          <tbody>{rows.map(row => <tr key={row.id} className={!row.enabled ? 'disabled' : ''}>
            ...
            {unit.code === 'CENTRAL' && <td><input value={row.category} onChange={event => patchRow(row.id, { category: event.target.value })}/></td>}
