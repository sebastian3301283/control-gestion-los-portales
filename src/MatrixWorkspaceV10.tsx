...
export default function MatrixWorkspaceV10({ periodId, year, unitCode, unitName, canManage, onError, onNotice }: Props) {
  ...
  const groupOptions = unitCode === 'CENTRAL' ? centralGuidelineGroups : rowObjectiveGroups
  ...
  async function exportExcel() {
    ...
    try {
      ...
      const exportRows = rows.map((row, index) => ({ ...(showNumberColumn ? { 'N°': index + 1 } : {}), ...(unitCode === 'CENTRAL' ? { Lineamiento: row.objective_group || '' } : { Objetivo: row.objective_group || '' }), Acción: row.objective || '', Responsable: row.responsible_manager_id ? managerById.get(row.responsible_manager_id)?.name || row.responsible_text || '' : row.responsible_text || '', Prioridad: row.priority || '', 'Hitos / Fechas': row.milestones || '', 'KPI (Cuantitativo)': row.kpi || '', Inicio: row.start_date || '', Fin: row.end_date || '', 'Riesgos de no ejecutar': row.risks || '', Restricciones: row.restrictions || '', Soporte: row.support || '', Entregable: row.deliverables || '', Comité: row.committee || '' }))
  ...
  async function importExcel(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      for (const row of grid.slice(headerIndex + 1)) {
        ...
        if (isGroupRow && !textValue(at(row, actionCol))) { currentGroup = joined; continue }
  ...
  return <div className={`matrix-v5 matrix-v10 matrix-v5--${unitAccent[unitCode]} ${page === 'sheet' ? 'matrix-v5--sheet' : ''} ${expanded ? 'matrix-v5--expanded' : ''}`}>
    ...
    {page === 'sheet' && selectedMatrix && <section className="matrix-v5-plan-shell">
      ...
      <div className="matrix-v5-sheet-card"><div className="matrix-v5-sheet-scroll" style={zoomStyle}><table className="matrix-v5-sheet"><thead><tr>{showNumberColumn && <th>N°</th>}<th>Acción</th><th>Responsable</th><th>Prioridad</th><th>Hitos / Fechas</th><th>KPI (Cuantitativo)</th><th>Inicio</th><th>Fin</th><th>Riesgos de no ejecutar</th><th>Restricciones</th><th>Soporte</th><th>Entregable</th><th>Comité</th>{effectiveCanManage && <th>Acciones</th>}</tr></thead><tbody>
