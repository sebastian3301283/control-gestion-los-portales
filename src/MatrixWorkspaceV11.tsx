...
export default function MatrixWorkspaceV11(props: Props) {
  ...
  useEffect(() => {
    ...
    const cloneInlineFieldCell = (originalCell: HTMLTableCellElement, proxyClass: string) => {
      const proxyCell = originalCell.cloneNode(true) as HTMLTableCellElement
      proxyCell.removeAttribute('rowspan')
      proxyCell.classList.add('matrix-v11-inline-proxy', proxyClass)
      const originalControl = originalCell.querySelector<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>('input,textarea,select')
      const proxyControl = proxyCell.querySelector<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>('input,textarea,select')
      if (originalControl && proxyControl) {
        proxyControl.value = originalControl.value
        const eventName = proxyControl instanceof HTMLSelectElement ? 'change' : 'input'
        proxyControl.addEventListener(eventName, () => {
          originalControl.value = proxyControl.value
          originalControl.dispatchEvent(new Event(eventName, { bubbles: true }))
        })
      }
      originalCell.classList.add('matrix-v11-inline-original-hidden')
      return proxyCell
    }

    const enhanceCentralInlineLayout = () => {
      if (props.unitCode !== 'CENTRAL') return
      root.querySelectorAll<HTMLTableElement>('.matrix-v5-sheet').forEach(table => {
        const headers = Array.from(table.querySelectorAll<HTMLTableCellElement>('thead th'))
        const objectiveHeader = headers.find(header => header.textContent?.trim() === 'Objetivo general')
        const subpointHeader = headers.find(header => header.textContent?.trim() === 'Subpunto')
        if (objectiveHeader && subpointHeader) {
          objectiveHeader.colSpan = 2
          subpointHeader.classList.add('matrix-v11-central-subpoint-header-hidden')
        }

        table.querySelectorAll<HTMLTableRowElement>('.matrix-v10-central-inline-header-row').forEach(headerRow => {
          if (headerRow.dataset.v11InlineAligned === 'true') return
          const filler = headerRow.querySelector<HTMLTableCellElement>('.matrix-v10-central-inline-header-fill')
          const firstDetailRow = headerRow.nextElementSibling as HTMLTableRowElement | null
          if (!filler || !firstDetailRow?.classList.contains('matrix-v10-central-inline-editor-row')) return

          const detailCells = Array.from(firstDetailRow.children) as HTMLTableCellElement[]
          const subpointIndex = detailCells.findIndex(cell => cell.classList.contains('matrix-v10-central-inline-subpoint-cell'))
          if (subpointIndex < 0) return

          const responsibleCell = detailCells[subpointIndex + 1]
          const priorityCell = detailCells[subpointIndex + 2]
          const risksCell = detailCells[subpointIndex + 7]
          const restrictionsCell = detailCells[subpointIndex + 8]
          const supportCell = detailCells[subpointIndex + 9]
          const deliverableCell = detailCells[subpointIndex + 10]
          const committeeCell = detailCells[subpointIndex + 11]
          const actionsCell = detailCells[subpointIndex + 12]
          if (!responsibleCell || !priorityCell || !risksCell || !restrictionsCell || !supportCell || !deliverableCell || !committeeCell) return

          const placeholder = () => {
            const cell = document.createElement('td')
            cell.className = 'matrix-v11-inline-detail-placeholder'
            return cell
          }

          const nextCells: HTMLTableCellElement[] = [
            cloneInlineFieldCell(responsibleCell, 'matrix-v11-inline-responsible'),
            cloneInlineFieldCell(priorityCell, 'matrix-v11-inline-priority'),
            placeholder(),
            placeholder(),
            placeholder(),
            placeholder(),
            cloneInlineFieldCell(risksCell, 'matrix-v11-inline-general-field'),
            cloneInlineFieldCell(restrictionsCell, 'matrix-v11-inline-general-field'),
            cloneInlineFieldCell(supportCell, 'matrix-v11-inline-general-field'),
            cloneInlineFieldCell(deliverableCell, 'matrix-v11-inline-general-field'),
            cloneInlineFieldCell(committeeCell, 'matrix-v11-inline-general-field'),
          ]
          if (actionsCell) nextCells.push(cloneInlineFieldCell(actionsCell, 'matrix-v11-inline-actions'))
          filler.replaceWith(...nextCells)
          headerRow.dataset.v11InlineAligned = 'true'
        })
      })
    }

    const enhanceLayout = () => {
      enhanceCentralInlineLayout()
      root.querySelectorAll<HTMLTableElement>('.matrix-v5-sheet').forEach(table => {
